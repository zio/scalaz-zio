...
object Gen extends GenZIO with FunctionVariants with TimeVariants {
  ...
  /**
   * Combines the specified generators to return a new generator that generates
   * all the values generated by the specified generators. For infinite
   * generators, it will generate values from a bounded prefix determined by the
   * current size parameter.
   */
  def concatAll[R, A](gs: Iterable[Gen[R, A]])(implicit trace: Trace): Gen[R, A] =
    if (gs.isEmpty) empty
    else
      Gen {
        ZStream.repeatZIO {
